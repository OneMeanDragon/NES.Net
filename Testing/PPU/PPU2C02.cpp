...
PPU2C02::PPU2C02()
    : _cart(nullptr), _pixelCallback(nullptr)
...
uint8_t PPU2C02::PpuRead(uint16_t addr, bool rdOnly) {
    ...
    if (addr <= 0x1FFF) {
        ...
        MirrorMode mirror = (_cart ? (MirrorMode)_cart->GetMirrorMode() : MirrorMode::Horizontal);
...
void PPU2C02::Clock()
{
    ...
    // ============================================================
    // PRE-RENDER & VISIBLE SCANLINES (-1 to 239)
    // ============================================================
    if (_scanline >= -1 && _scanline <= 239)
    {
        ...
        // --------------------------------------------------------
        // BACKGROUND FETCH PIPELINE (cycles 1–256, 321–336)
        // --------------------------------------------------------
        if (renderingEnabled &&
            ((_cycle >= 2 && _cycle <= 257) ||
                (_cycle >= 322 && _cycle <= 337)))
        {
            //UpdateShifters();
            ...
            case 1: // Name table byte
                ...
                _bgNextTileId =
                    PpuRead(0x2000 | (_vramAddr.reg & 0x0FFF));
                break;
            ...
            case 3: // Attribute table byte
            {
                ...
                uint8_t shift =
                    ((_vramAddr.coarseY >> 1) & 0x02) |
                    ((_vramAddr.coarseX >> 2) & 0x01);
                shift <<= 1;
            ...
            case 7: {// Pattern high byte
                uint16_t tileAddr =
                    (_control.patternBackground ? 0x1000 : 0x0000) |
                    (_bgNextTileId << 4) |
                    _vramAddr.fineY;

                _bgNextTileMsb = PpuRead(tileAddr + 8);
                break;
            }

            case 0: // Increment coarse X
                IncrementScrollX();
                break;
            }
        }
        ...
        // SPRITE EVALUATION
        // --------------------------------------------------------
        if (renderingEnabled && _cycle == 257 && _scanline >= 0)
            EvaluateSprites();
    ...
    // ============================================================
    // Idle scanline - no rendering, no logic
    ...
    if (_mask.renderBackground &&
        (_mask.renderBackgroundLeft || _cycle >= 9))
    {
        uint16_t bit = 0x8000 >> _fineX;

        bgPixel =
            (((_bgShifterPatternHi & bit) != 0) << 1) |
            ((_bgShifterPatternLo & bit) != 0);

        bgPalette =
            (((_bgShifterAttribHi & bit) != 0) << 1) |
            ((_bgShifterAttribLo & bit) != 0);
    }

    // Sprites
    if (_mask.renderSprites &&
        (_mask.renderSpritesLeft || _cycle >= 9))
    {
        _spriteZeroBeingRendered = false;
        ...
        for (uint8_t i = 0; i < _spriteCount && i < 8; i++)
        {
            ...
            {
                fgPixel =
                    (((_spriteShifterHi[i] & 0x80) != 0) << 1) |
                    ((_spriteShifterLo[i] & 0x80) != 0);

                if (fgPixel)
                {
                    fgPalette =
                        (_spriteScanline[i].attribute & 0x03) + 4;

                    fgPriority =
                        !(_spriteScanline[i].attribute & 0x20);
    ...
    else
    {
        ...
        // CRITICAL FIX: Sprite zero hit respects BOTH clipping flags
    ...
    // Output - FIXED: Only render when rendering is enabled
        ...
        _cycle >= 1 && _cycle <= 256)
    {
        if (_pixelCallback)
        {
            ...
            //_pixelCallback(_cycle - 1, _scanline, _cycle - 1, _scanline, 255); //out put to the renderer is good
            _pixelCallback(_cycle - 1, _scanline, c.r, c.g, c.b);
        }
    }

    // After pixel color is computed, before cycle++
    if (renderingEnabled && _cycle >= 1 && _cycle <= 256)
    {
        UpdateShifters();
    }
    ...
    if (_cycle == 260 && renderingEnabled && _scanline < 240)
    {
...
DLLEXPORT void PPU_GetOAMEntry(PPU2C02* ppu, uint8_t index, PPU2C02::OAMEntry* entry) {
    if (ppu && entry && index < 64) {
        *entry = ppu->OAM[index];  // Simple assignment
    }
}

DLLEXPORT void PPU_SetOAMEntry(PPU2C02* ppu, uint8_t index, PPU2C02::OAMEntry* entry) {
    if (ppu && entry && index < 64) {
        ppu->OAM[index] = *entry;  // Simple assignment
    }
}

DLLEXPORT uint8_t PPU_GetOAMByte(PPU2C02* ppu, uint8_t oamAddr) {
    if (!ppu) return 0xFF;  // 256 bytes max
    return reinterpret_cast<uint8_t*>(ppu->OAM)[oamAddr];
}

DLLEXPORT void PPU_SetOAMByte(PPU2C02* ppu, uint8_t oamAddr, uint8_t data) {
    if (!ppu) return;  // 256 bytes max
    reinterpret_cast<uint8_t*>(ppu->OAM)[oamAddr] = data;
}
